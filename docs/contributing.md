...
01. Fork the `mellanox-lab/libxlio` repo.

02. Make your changes in a new git branch:

    ```shell
    git checkout -b fix-branch master
    ```

03. Do relevant modifications and include appropriate unit-test cases.

04. Follow our [Coding Rules](#rules).

05. Run the existing test suite as described in the [Test suite](#tests), and ensure that all tests pass.

06. Prepare sequence of small commits that as one self-contained change. Read [Why Write Small Commit](#patch).

07. Commit your changes using a descriptive commit message that follows our [Commit Message Conventions](#commit).

    ```shell
    git commit -a -s
    ```

08. Once you have committed your changes, it is a good idea to use `git rebase`
